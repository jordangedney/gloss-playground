...
data GridGame = Game
  ...
  , _previouslyProcessed :: DisplayableShape
  , _frameNumber :: Int
  } deriving (Show)
makeLenses ''GridGame
...
initialState = Game
  ...
  , _wall = coloredWall
  , _nodesToProcess = Q.push centerNode Q.emptyQueue
  , _previouslyProcessed = centerNode
  , _frameNumber = 0
  }
  where square = DisplayableShape
        ...
        centerNode = fromJust $ Map.lookup (0, 0) center
...
neighborNodes nodeList node
  ...
  where (nodeX, nodeY) = (node ^. entityAttrs . coords . _1,
        ...
        neighborCoords = [upCoords, leftCoords, rightCoords, downCoords]
...
breadthFirstSearch gameState =
  case top of
    ...
    Just element ->
      gameState
      & setPreviouslyProcessedToColor (greyN 0.5)
      & replacePreviouslyProcessed
      & addFrontierToProcessed
      & replaceQueue
      & removeProcessedFromUnprocessed
      & setPreviouslyProcessedToColor white
      where neighbors = neighborNodes (gameState ^. unprocessed) element

            replacePreviouslyProcessed game = game & previouslyProcessed .~ element

            addFrontierToProcessed game =
              game & processed .~
              (Map.union (Map.union (game ^. processed) $ toSet [element])
                         neighbors)

            replaceQueue game = game & nodesToProcess .~ queueWithNeighbors
              where queueWithNeighbors :: Q.Queue DisplayableShape
                    queueWithNeighbors =
                      foldl (\l r -> Q.push r l) newQueue (Map.elems neighbors)

            removeProcessedFromUnprocessed game =
              game & unprocessed .~
              (Map.difference (game ^. unprocessed) (game ^. processed))

  where (top, newQueue) = Q.pop $ gameState ^. nodesToProcess

        replaceQueue game = game & nodesToProcess .~ newQueue

        setPreviouslyProcessedToColor color_ game =
          game & over processed
                      (Map.update (\x -> Just $ setNodeColor color_ x)
                                  (game ^. previouslyProcessed
                                         . entityAttrs
                                         . coords))

setNodeColor :: Color -> DisplayableShape -> DisplayableShape
setNodeColor color_ node = node & visualAttrs . objectColor .~ color_
...
processWave gameState =
  ...
  & over frontier (Map.map $ setNodeColor $ white)
  & removeProcessedFromFrontier
  & over processed (Map.map $ setNodeColor $ greyN 0.2)
  where
        addFrontierToProcessed game =
          game & processed .~
...
resetIfDone :: GridGame -> GridGame
resetIfDone game =
  if (length $ Map.elems (game ^. unprocessed)) == 0
  then initialState
  else game
...
main = play window background framesPerSecond initialState render handleKeys update
  where
    ...
    update secondsSinceLastUpdate
      = everyXFrames runSpeed breadthFirstSearch .
        -- debug .
        -- resetIfDone .
